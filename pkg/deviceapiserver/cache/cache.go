...
// setMetadataResourceVersion sets the resource version in the GPU's metadata.
// This ensures metadata is properly initialized and the version is set as a string
// following K8s API conventions.
func setMetadataResourceVersion(gpu *v1alpha1.Gpu, version int64) {
	if gpu.Metadata == nil {
		gpu.Metadata = &v1alpha1.ObjectMeta{}
	}
	gpu.Metadata.ResourceVersion = strconv.FormatInt(version, 10)
}
...
func (c *GpuCache) Set(gpu *v1alpha1.Gpu) int64 {
	...
	setMetadataResourceVersion(gpuCopy, c.resourceVersion)
